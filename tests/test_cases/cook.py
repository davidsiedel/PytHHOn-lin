...
def solve(
    ...
    for cell_index in cells_indices:
        ...
        # total_size = 3 * cell_basis_k.basis_dimension
        ...
        # gg0 = local_gradient_operator[2 * cell_basis_k.basis_dimension : 3 * cell_basis_k.basis_dimension, :]
        # gg1 = local_gradient_operator[3 * cell_basis_k.basis_dimension : 4 * cell_basis_k.basis_dimension, :]
        # print("grad_res : {}".format(gg0 - gg1))
        # local_gradient_operator_eff = local_gradient_operator[:total_size, :]
        # local_gradient_operator_eff[2 * cell_basis_k.basis_dimension :, :] = np.sqrt(2.0) * (
        #     local_gradient_operator[2 * cell_basis_k.basis_dimension : 3 * cell_basis_k.basis_dimension, :]
        #     + local_gradient_operator[3 * cell_basis_k.basis_dimension : 4 * cell_basis_k.basis_dimension, :]
        # )
        local_stiffness_form = local_gradient_operator.T @ local_mass_matrix @ local_gradient_operator
        # local_stiffness_form = local_gradient_operator_eff.T @ local_mass_matrix @ local_gradient_operator_eff
        # --------------------------------------------------------------------------------------------------------------
    ...
    for cell_index in cells_indices:
        ...
        for i, quadrature_point in enumerate(local_cell.quadrature_points):
            ...
            for direction in range(unknown.field_dimension):
                ...
                # ----------
                stress[direction] += (2 * mu + lam) * np.sum(v_k * grad_vect[l0_k:l1_k])
                strain[direction] += np.sum(v_k * grad_vect[l0_k:l1_k])
            # --------------
            stress[2] += 0.0
            strain[2] += lam / (lam + 2.0 * mu) * (strain[0] + strain[1])
            # --------------
            l0_k = 2 * cell_basis_k.basis_dimension
            l1_k = (2 + 1) * cell_basis_k.basis_dimension
            stress[3] += np.sqrt(2) * (2 * mu) * np.sum(v_k * grad_vect[l0_k:l1_k])
            strain[3] += np.sqrt(2) * np.sum(v_k * grad_vect[l0_k:l1_k])
            # --------------
            stress[4] += 0.0
            strain[4] += 0.0
            # --------------
            stress[5] += 0.0
            strain[5] += 0.0
            stress_at_quadrature_points.append(stress)
            strain_at_quadrature_points.append(strain)
            # ----------------------------------------------------------------------------------------------------------
            point_A = (48.0, 52.0)
