...
def solve_2D_incompressible_problem(
    ...
    field_dimension = 1
    mesh_file = os.path.join(source, "meshes/2D/c2d4_{}_square.geof".format(number_of_elements))
    # create_plot(mesh_file, number_of_elements)
    # create_plot_2(mesh_file, number_of_elements)
    # ------------------------------------------------------------------------------------------------------------------
    pressure_left = [None]
    pressure_right = [None]
    pressure_top = [None]
...
def create_plot_2(file_path: str, number_of_elements):
    with open(file_path, "w") as mesh_file:
        number_of_nodes = (number_of_elements + 1) ** 2
        nb_nd = number_of_elements + 1
        mesh_file.write("***geometry\n**node\n{} 2\n".format(number_of_nodes))
