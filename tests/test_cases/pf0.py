...
def solve(
    ...
    for cell_index in cells_indices:
        ...
        # MASS matrix
        # --------------------------------------------------------------------------------------------------------------
        total_size = unknown.field_dimension * cell_basis_l.basis_dimension
        local_id_matrix = np.zeros((total_size, total_size))
        m_phi_phi_cell = Integration.get_cell_mass_matrix_in_cell(local_cell, cell_basis_l, cell_basis_l)
        for i in range(unknown.field_dimension):
            r0 = i * cell_basis_l.basis_dimension
            r1 = (i + 1) * cell_basis_l.basis_dimension
            local_id_matrix[r0:r1, r0:r1] += m_phi_phi_cell
        local_mass_operator = operators[cell_index].local_mass_operator
        local_id_operator = local_mass_operator.T @ local_id_matrix @ local_mass_operator
        # --------------------------------------------------------------------------------------------------------------
        ...
        # --------------------------------------------------------------------------------------------------------------
        local_matrix = (
            G_c / (2.0 * l) * (local_stiffness_form + (1.0 / l ** 2) * local_id_operator + local_stabilization_operator)
        )
        # --------------------------------------------------------------------------------------------------------------
        # Static condensation
        # --------------------------------------------------------------------------------------------------------------
...
def solve_2D_incompressible_problem(
    ...
    field_dimension = 1
    mesh_file = os.path.join(source, "meshes/2D/c2d3_{}_HHO.geof".format(number_of_elements))
    # create_plot(mesh_file, number_of_elements)
    triangles = create_plot_HHO(mesh_file, number_of_elements)
    # ------------------------------------------------------------------------------------------------------------------
    pressure_left = [None]
    pressure_right = [None]
    pressure_top = [None]
    ...
    boundary_conditions = {
        ...
        # --------------------------------------------------------------------------------------------------------------
        # "MIDDLE": ([None], pressure_bottom),
        # "HLEFT": ([None], pressure_bottom),
        # "HRIGHT": ([None], pressure_bottom),
        # "OTOP": ([None], pressure_bottom),
        # "OBOTTOM": ([None], pressure_bottom),
        # "O": ([lambda x: 1.0], pressure_bottom),
        # --------------------------------------------------------------------------------------------------------------
        # "MIDDLE": ([lambda x: 1.0], pressure_bottom),
        # "HLEFT": ([lambda x: 1.0], pressure_bottom),
        # "HRIGHT": ([lambda x: 1.0], pressure_bottom),
        # "OTOP": ([None], pressure_bottom),
        # "OBOTTOM": ([None], pressure_bottom),
        # "O": ([None], pressure_bottom),
        # --------------------------------------------------------------------------------------------------------------
        "HSETI": ([lambda x: 1.0], pressure_bottom),
        "HSETII": ([lambda x: 1.0], pressure_bottom),
        "O": ([lambda x: 1.0], pressure_bottom),
    }
    # ------------------------------------------------------------------------------------------------------------------
    (
    ...
    colors = [(0, 0, 0), (1, 0.6470588235294118, 0), (1, 0, 0)]  # Halloween
    ...
    cbar.set_label("{} phase field solution".format(operator_type), rotation=270, labelpad=15.0)
    # cbar.ax0.set_yticklabels(["0.0", "0.5", "1.0"])
    # ==================================================================================================================
...
def create_plot_HHO(file_path: str, number_of_elements):
    with open(file_path, "w") as mesh_file:
        # number_of_nodes = (number_of_elements + 1) ** 2
        number_of_nodes = (number_of_elements + 1) * (3 * number_of_elements + 1)
        mesh_file.write("***geometry\n**node\n{} 2\n".format(number_of_nodes))
        segmentation_X = np.linspace(0.0, 3.0, (3 * number_of_elements) + 1)
        segmentation_Y = np.linspace(0.0, 1.0, number_of_elements + 1)
        number_of_elements_X = 3 * number_of_elements
        number_of_elements_Y = number_of_elements
        count = 1
        for point_Y in segmentation_Y:
            for point_X in segmentation_X:
                mesh_file.write("{} {} {}\n".format(count, point_X, point_Y))
                count += 1
        nb_el = 2 * (number_of_elements_X * number_of_elements_Y)
        mesh_file.write("**element\n{}\n".format(nb_el))
        count = 1
        nb_nd_in_row = 3 * number_of_elements + 1
        triangles = []
        for count_row in range(number_of_elements_Y):
            for count_node in range(number_of_elements_X):
                count_node_eff = count_node + 1
                count_row_eff = count_row
                # ------------------------------------------------------------------------------------------------------
                p1 = count_node_eff + (count_row_eff * nb_nd_in_row)
                p2 = count_node_eff + 1 + (count_row_eff * nb_nd_in_row)
                p3 = count_node_eff + nb_nd_in_row + (count_row_eff * nb_nd_in_row)
                mesh_file.write("{} c2d3 {} {} {}\n".format(count, p1, p2, p3))
                count += 1
                triangles.append([p1 - 1, p2 - 1, p3 - 1])
                # ------------------------------------------------------------------------------------------------------
                q1 = count_node_eff + 1 + (count_row_eff * nb_nd_in_row)
                q2 = count_node_eff + nb_nd_in_row + (count_row_eff * nb_nd_in_row)
                q3 = count_node_eff + nb_nd_in_row + 1 + (count_row_eff * nb_nd_in_row)
        ...
        # --------------------------------------------------------------------------------------------------------------
        mesh_file.write("\n")
        mesh_file.write("**nset HSETI\n")
        count = 1
        for point_Y in segmentation_Y:
            for point_X in segmentation_X:
                if point_X > 0.09999999 and point_X < 0.90000001 and point_Y > 0.49999999 and point_Y < 0.50000001:
                    mesh_file.write(" {}".format(count))
                if point_X > 0.09999999 and point_X < 0.10000001 and point_Y > 0.09999999 and point_Y < 0.90000001:
                    mesh_file.write(" {}".format(count))
                if point_X > 0.89999999 and point_X < 0.90000001 and point_Y > 0.09999999 and point_Y < 0.90000001:
                    mesh_file.write(" {}".format(count))
                count += 1
        # --------------------------------------------------------------------------------------------------------------
        mesh_file.write("\n")
        mesh_file.write("**nset HSETII\n")
        count = 1
        for point_Y in segmentation_Y:
            for point_X in segmentation_X:
                if point_X > 1.09999999 and point_X < 1.90000001 and point_Y > 0.49999999 and point_Y < 0.50000001:
                    mesh_file.write(" {}".format(count))
                if point_X > 1.09999999 and point_X < 1.10000001 and point_Y > 0.09999999 and point_Y < 0.90000001:
                    mesh_file.write(" {}".format(count))
                if point_X > 1.89999999 and point_X < 1.90000001 and point_Y > 0.09999999 and point_Y < 0.90000001:
                    mesh_file.write(" {}".format(count))
                count += 1
        # --------------------------------------------------------------------------------------------------------------
        mesh_file.write("\n")
        mesh_file.write("**nset O\n")
        count = 1
        for point_Y in segmentation_Y:
            for point_X in segmentation_X:
                if (point_X - 2.5) ** 2 + (point_Y - 0.5) ** 2 > (0.370001) ** 2 and (point_X - 2.5) ** 2 + (
                    point_Y - 0.5
                ) ** 2 < (0.40000001) ** 2:
                    mesh_file.write(" {}".format(count))
                count += 1
...
def create_plot(file_path: str, number_of_elements):
    with open(file_path, "w") as mesh_file:
        ...
        # --------------------------------------------------------------------------------------------------------------
        mesh_file.write("\n")
        mesh_file.write("**nset MIDDLE\n")
        count = 1
        for point_Y in segmentation:
            # if point_Y > 0.49999999 and point_Y < 0.50000001:
            for point_X in segmentation:
                if point_X > 0.09999999 and point_X < 0.90000001 and point_Y > 0.49999999 and point_Y < 0.50000001:
                    mesh_file.write(" {}".format(count))
                count += 1
        # --------------------------------------------------------------------------------------------------------------
        mesh_file.write("\n")
        mesh_file.write("**nset OTOP\n")
        count = 1
        for point_Y in segmentation:
            # if point_Y > 0.49999999 and point_Y < 0.50000001:
            for point_X in segmentation:
                if point_X > 0.09999999 and point_X < 0.90000001 and point_Y > 0.89999999 and point_Y < 0.90000001:
                    mesh_file.write(" {}".format(count))
                count += 1
        # --------------------------------------------------------------------------------------------------------------
        mesh_file.write("\n")
        mesh_file.write("**nset OBOTTOM\n")
        count = 1
        for point_Y in segmentation:
            # if point_Y > 0.49999999 and point_Y < 0.50000001:
            for point_X in segmentation:
                if point_X > 0.09999999 and point_X < 0.90000001 and point_Y > 0.09999999 and point_Y < 0.10000001:
                    mesh_file.write(" {}".format(count))
                count += 1
        # --------------------------------------------------------------------------------------------------------------
        mesh_file.write("\n")
        ...
        for point_Y in segmentation:
            for point_X in segmentation:
                if (point_X - 0.5) ** 2 + (point_Y - 0.5) ** 2 > (0.370001) ** 2 and (point_X - 0.5) ** 2 + (
                    point_Y - 0.5
                ) ** 2 < (0.40000001) ** 2:
                    mesh_file.write(" {}".format(count))
                count += 1
