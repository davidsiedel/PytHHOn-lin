...
def solve(
    ...
    # div_at_quadrature_points = [np.zeros((number_of_quadrature_points,)) for i in range(unknown.field_dimension)]
...
def solve_2D_incompressible_problem(
    ...
    # create_plot_cooke(mesh_file, number_of_elements)
    ...
    # displacement_appli = [None, lambda x: 1.2]
    ...
    # fig, (ax0, ax0s) = plt.subplots(nrows=1, ncols=2, sharex=True)
    fig, (ax0, ax0s, ax0d) = plt.subplots(nrows=1, ncols=3, sharex=True)
    ...
    # data = ax0d.tricontourf(x, y, div_at_quadrature_points, cmap=cm.binary)
    # data = ax0d.tricontourf(x, y, lam * div_at_quadrature_points, cmap=cm.binary)
    ...
    cbar = fig.colorbar(data, ax=ax0d)
    ...
    # data = ax0.tricontourf(x, y, div_at_quadrature_points, cmap=cm.binary)
    # data = ax0.tricontourf(x, y, unknowns_at_vertices[0], cmap=cm.binary, levels=np.linspace(0.0, 1.0, 1000))
    # data = ax0.tricontourf(x, y, unknowns_at_vertices[0], cmap=cm.binary)
    # data = ax.tricontourf(x, y, unknowns_at_quadrature_points[1], cmap=cm.binary)
    # data = ax.tricontourf(x, y, unknowns_at_quadrature_points[0], vmin=vmin, vmax=vmax, cmap=cm.binary)
    ...
    # ------------------------------------------------------------------------------------------------------------------
    data = ax0s.tricontourf(x, y, unknowns_at_quadrature_points[:, 1], cmap=cm.binary)
    ax0s.triplot(triang, linewidth=0.2, color="grey")
    # data = ax0.tricontourf(x, y, unknowns_at_vertices[0], cmap=cm.binary, levels=np.linspace(0.0, 1.0, 1000))
    # data = ax0.tricontourf(x, y, unknowns_at_vertices[0], cmap=cm.binary)
    # data = ax.tricontourf(x, y, unknowns_at_quadrature_points[1], cmap=cm.binary)
    # data = ax.tricontourf(x, y, unknowns_at_quadrature_points[0], vmin=vmin, vmax=vmax, cmap=cm.binary)
    # ax0s.set_xlim(0.0, 1.0)
    # ax0s.set_ylim(0.0, 1.0)
    ax0s.get_xaxis().set_visible(False)
    ax0s.get_yaxis().set_visible(False)
    ax0s.set_xlabel("map of the domain $\Omega$")
    # ax0s.set_title(
    #     "{}, $k = {}, l = {}$, stab $= {}$".format(
    #         operator_type, face_polynomial_order, cell_polynomial_order, stabilization_parameter
    #     )
    # )
    ax0s.set_title("{}, $k = {}, l = {}$".format(operator_type, face_polynomial_order, cell_polynomial_order))
    cbar = fig.colorbar(data, ax=ax0s)
    ...
    # Z_X = np.sin(np.pi * X) * np.sin(np.pi * Y) + 1.0 / (2.0 * lam) * X
    # Z_Y = np.cos(np.pi * X) * np.cos(np.pi * Y) + 1.0 / (2.0 * lam) * Y
    ...
    # CS = ax1.contourf(X, Y, Z_X, col_levels, cmap=cm.binary)
...
def create_plot_cooke(file_path: str, number_of_elements):
    with open(file_path, "w") as mesh_file:
        ...
        for point_Y in segmentation:
            for point_X in segmentation:
                X = 48.0 * point_X
                # Y = (1.0 - point_Y) * 44.0 * point_X + point_Y * 16.0 * point_X + 44.0 * point_Y
