...
def throw(
    ...
    for cell_index in cells_indices:
        ...
        for local_quad_point, local_quad_weight in zip(local_cell.quadrature_points, local_cell.quadrature_weights):
            ...
            t = material_data_manager.K[quad_point_count]
            # print("tangent_matrix_mgis : \n{}".format(tangent_matrix_mgis))
            tangent_matrix = t[[[0], [1], [3]], [0, 1, 3]]
            t_rec = np.zeros((4, 4))
            t_rec[:3, :3] += tangent_matrix
            t_rec[3, 3] += t_rec[2, 2]
            t_eff = tangent_matrix
            # ----------------------------------------------------------------------------------------------------------
            ...
            for i in range(t_eff.shape[0]):
                for j in range(t_eff.shape[1]):
                    # if i == 2 and j == 2:
                    #     coef = 2.0
                    # else:
                    #     coef = 1.0
                    m = t_eff[i, j] * m_phi_phi_cell
                    # --------------------------------------------------------------------------------------------------
            ...
            local_gradient_operator = get_symmetric_grad(local_gradient_operator_init, cell_basis_k)
    ...
    return global_matrix_2, global_vector_2, stored_matrices
...
def solve_newton(
    ...
    total_system_size = number_of_faces * face_basis_k.basis_dimension * unknown.field_dimension
    total_system_size = (
        number_of_faces * face_basis_k.basis_dimension + number_of_cells * cell_basis_l.basis_dimension
    ) * unknown.field_dimension
    # ------------------------------------------------------------------------------------------------------------------
    # initialization
    # ------------------------------------------------------------------------------------------------------------------
    ...
    for pressure_step, load_step in zip(pressure_list, load_list):
        ...
        int_forces_at_iteration = np.zeros((total_system_size,))
        res_at_iteration = np.zeros((total_system_size,))
        for mech_iteration in range(1):
            ...
            mgis_bv.setExternalStateVariable(m.s1, "Temperature", T, Ts)
            # ----------------------------------------------------------------------------------------------------------
            # global_matrix_2, global_vector_2, stored_matrices = throw(
            _, _, stored_matrices = throw(
                vertices,
                faces,
                cells,
                operators,
            ...
            for cell_index in range(number_of_cells):
                ...
                for local_index_col, global_index_col in enumerate(cell_faces_connectivity_matrix):
                    ...
                    l0c = local_index_col * face_basis_k.basis_dimension * unknown.field_dimension
                    l1c = (local_index_col + 1) * face_basis_k.basis_dimension * unknown.field_dimension
                    x_faces[l0c:l1c] += delta_unknown_at_iteration[g0c:g1c]
                ...
                for q_p in local_cell.quadrature_points:
                    grad_op_eff = get_symmetric_grad(grad_op, cell_basis_k)
                    b_mat = compute_strain_operator(cell_basis_k, grad_op_eff, local_cell, q_p)
                    strain = b_mat @ x_element.T
                    m.s1.gradients[q_p_count] = strain
                    q_p_count += 1
            ...
            for cell_index in range(number_of_cells):
                ...
                local_faces = [faces[i] for i in cell_faces_connectivity_matrix]
                faces_unknown_dimension = len(local_faces) * face_basis_k.basis_dimension * unknown.field_dimension
                cell_unknown_dimension = cell_basis_k.basis_dimension * unknown.field_dimension
                internal_force = np.zeros((cell_unknown_dimension + faces_unknown_dimension,))
                for q_p in local_cell.quadrature_points:
                    grad_op = operators[cell_index].local_gradient_operator
                    b_mat = compute_strain_operator(cell_basis_k, grad_op_eff, local_cell, q_p)
                    internal_force += b_mat.T @ m.s1.thermodynamic_forces[q_p_count]
                cell_faces_connectivity_matrix = cells_faces_connectivity_matrix[cell_index]
                local_faces = [faces[i] for i in cell_faces_connectivity_matrix]
                faces_unknown_dimension = len(local_faces) * face_basis_k.basis_dimension * unknown.field_dimension
                x_faces = np.zeros((faces_unknown_dimension,))
                ...
                for q_p in local_cell.quadrature_points:
                    ...
                    b_mat = compute_strain_operator(cell_basis_k, grad_op_eff, local_cell, q_p)
                    strain = b_mat @ x_element.T
                    m.s1.thermodynamic_forces[q_p_count] = strain
                    q_p_count += 1
            # mgis_bv.integrate(m, it, 0.0, 0, m.n)  # args are(mat_data, integ_type, dt=0.0, start = 0, end = m.n)
...
def compute_strain_operator(cell_basis_k: Basis, grad_op: Mat, cell: Cell, quad_point: Mat):
    v_k = cell_basis_k.get_phi_vector(quad_point, cell.centroid, cell.diameter)
    dim_grad = grad_op.shape[0]
    dim_elem = grad_op.shape[1]
    dim_strain = 4
    m = np.zeros((dim_strain, dim_grad))
    c0 = 0 * cell_basis_k.basis_dimension
    c1 = 1 * cell_basis_k.basis_dimension
    m[0, c0:c1] += v_k
    c0 = 1 * cell_basis_k.basis_dimension
    c1 = 2 * cell_basis_k.basis_dimension
    m[1, c0:c1] += v_k
    r0 = 2 * cell_basis_k.basis_dimension
    r1 = 3 * cell_basis_k.basis_dimension
    m[3, r0:r1] += v_k
    # return grad_op.T @ m.T
    return m @ grad_op


def get_symmetric_grad(local_gradient_operator_init: Mat, cell_basis_k: Basis):
    r0 = 0 * (cell_basis_k.basis_dimension)
    r1 = 3 * (cell_basis_k.basis_dimension)
    local_gradient_operator = local_gradient_operator_init[r0:r1, :]
    r0 = 2 * (cell_basis_k.basis_dimension)
    r1 = 3 * (cell_basis_k.basis_dimension)
    R1 = 4 * (cell_basis_k.basis_dimension)
    local_gradient_operator[r0:r1, :] = np.sqrt(2.0) * (
        local_gradient_operator_init[r0, r1] + local_gradient_operator_init[r1, R1]
    )
