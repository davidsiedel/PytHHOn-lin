...
def solve(
    ...
    for cell_index in cells_indices:
        ...
        for i, quadrature_point in enumerate(local_cell.quadrature_points):
            print(len(local_cell.quadrature_points))
...
def solve_2D_incompressible_problem(
    ...
    field_dimension = 2
    mesh_file = os.path.join(source, "meshes/2D/c2d3_{}_cooke_poly.geof".format(number_of_elements))
    create_plot_cooke_2(mesh_file, number_of_elements)
    # ------------------------------------------------------------------------------------------------------------------
    ...
    fig2, (ax0d) = plt.subplots(nrows=1, ncols=1, sharex=True)
    # ==================================================================================================================
    ...
    quantity = (1.0 / 3.0) * (2.0 * mu + lam) * div_at_quadrature_points[:]
    quantity = div_at_quadrature_points[:]
    ...
    datad = ax0d.tricontourf(x, y, quantity, cmap=perso, levels=levels)
    ...
    # cbar.set_label("hydrostatic pressure [MPa]".format(operator_type), rotation=270, labelpad=15.0)
    # ------------------------------------------------------------------------------------------------------------------
...
def create_plot_cooke_2(file_path: str, number_of_elements):
    with open(file_path, "w") as mesh_file:
        number_of_nodes = (number_of_elements + 1) ** 2
        nb_nd = number_of_elements + 1
        mesh_file.write("***geometry\n**node\n{} 2\n".format(number_of_nodes))
        ...
        for point_Y in segmentation:
            for point_X in segmentation:
                ...
                mesh_file.write("{} {} {}\n".format(count, X, Y))
                count += 1
        nb_el = number_of_elements * number_of_elements
        mesh_file.write("**element\n{}\n".format(nb_el))
        count = 1
        for count_node in range(number_of_elements):
            for count_line in range(number_of_elements):
                ...
                p2 = count_node_eff + 1 + (count_line_eff * nb_nd)
                p3 = count_node_eff + nb_nd + 1 + (count_line_eff * nb_nd)
                p4 = count_node_eff + nb_nd + (count_line_eff * nb_nd)
                # mesh_file.write("{} c2d4 {} {} {} {}\n".format(count, p1, p4, p3, p2))
                mesh_file.write("{} c2d4 {} {} {} {}\n".format(count, p1, p2, p3, p4))
                count += 1
        eps = 0.001
        mesh_file.write("***group\n")
        mesh_file.write("**nset TOP\n")
...
operator_type = "HDGs"
stabilization_parameter = 2.0e6 * mu
